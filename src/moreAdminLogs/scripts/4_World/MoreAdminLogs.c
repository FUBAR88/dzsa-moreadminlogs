...
class MoreAdminLogs extends PluginBase
{
	...
	void openLogFile() {
		string baseDir = "$profile:MoreAdminLogs";
		string date = GetDateStr("-");
		string time = GetTimeStr("-");
		string path = string.Format("%1/adm_%2_%3", baseDir, date, time);
		if (IsJson()) path = path + ".ljson";
		else path = path + ".log";
		
		// Try to create the log file - if directory doesn't exist, it will fail
		this._logFile = OpenFile(path, FileMode.WRITE);
		
		// If file creation failed, try the old location as fallback
		if (!this._logFile) {
			string fallbackPath = string.Format("$profile:adm_%1_%2", date, time);
			if (IsJson()) fallbackPath = fallbackPath + ".ljson";
			else fallbackPath = fallbackPath + ".log";
			this._logFile = OpenFile(fallbackPath, FileMode.WRITE);
		}
	}
